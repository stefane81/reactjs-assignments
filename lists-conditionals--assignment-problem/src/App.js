...
class App extends Component {
  ...
  deleteCharHandler = index => {
    ...
    this.setState({
      inputText: charString,
      inputTextLength: charString.length
    });
  };
  ...
  render() {
    ...
    let charMap = (
      <div>
        {charArr.map((_char, index) => {
          return (
            <CharComponent
              char={_char}
              key={this.getRandomInt(0, 99999999999999, _char)}
    ...
    return (
      <div className="App">
        <input id="inputBox" value={this.state.inputText} key="input1" onChange={event => this.inputTextLength(event)} />
